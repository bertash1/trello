...
export const deleteCard = async (
  ...
  await Card.updateMany(
    { board: card?.board, position: { $gt: card?.position } },
    { $inc: { position: -1 } }
  )
  await card?.remove()
...
export const changeOrder = async (
  ...
  const {
    ...
    oldPosition,
    boardId,
  }: { newPosition: number; oldPosition: number; boardId: string } = req.body
  ...
  if (newPosition - oldPosition < -1) {
    await Card.updateMany(
      { board: boardId, position: { $gte: newPosition, $lt: oldPosition } },
      { $inc: { position: 1 } }
    )
  ...
  if (newPosition - oldPosition > 1) {
    await Card.updateMany(
      { board: boardId, position: { $gte: oldPosition, $lte: newPosition } },
      { $inc: { position: -1 } }
    )
    const card = await Card.findByIdAndUpdate(
      cardId,
